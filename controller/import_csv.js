  ...
  load_csv: (req,res) => {
	...
	fs.createReadStream(req.file.path)
  	...
  	.on('end', () => {
        ...
        for(ele of results) {
          //console.log(ele);
          /* if(dups_list.find(name => name == ele['ONU Name'])) {
            ///console.log('dups = ' + ele['ONU Name']);
          } else { */
            if(ele['Receive Optical Power(dBm)'] != '--') {
              ...
              //console.log(_value);
              db.query(sql, _value , (error, _res)=>{
                if(error) {
            ...
            /* dups_list.push(ele['ONU Name']);
          } */
        }
        db.query("UPDATE aarx_master SET remark = '"+ nc_data + "', NRSSP = '"+ dups_list.length + "' WHERE id = " + master_id);
        res.render('show_upload', { upfile: req.file, qty : results.length, nc: nc_data, dup_count : dups_list.length });
      })
  	});
      }
    } catch(err) {
